...
from .utils import class_agnostic_nms, logger, find_all_spconv_keys, BATCH_SIZE, multi_classes_nms
from .mean_vfe import MeanVFE
from .voxel_backbone import VoxelBackBone8x, VoxelResBackBone8x
...
class SECONDNetIoU(nn.Module):
    ...
    def build_networks(self):
        ...
        # )
        dense_head_module = AnchorHeadMulti(
            model_cfg=self.model_cfg.DENSE_HEAD, 
            input_channels=model_info_dict['num_bev_features'], 
            num_class=self.num_class, 
            class_names=self.class_names, 
    ...
    def forward(self, batch_dict: Dict[str, torch.Tensor]):
        for index, cur_module in enumerate(self.module_list):
    ...
    def load_params_from_file(self, filename, to_cpu=False):
        ...
        # rospy.logerr([(k, model_state_disk[k].shape) for k in model_state_disk.keys() if "backbone_3d.conv_input" in k])
        # print(model_state_disk["backbone_3d.conv_input.0.weight"].shape)
        # print(model_state_disk["dense_head.reg_loss_func.code_weights"])
